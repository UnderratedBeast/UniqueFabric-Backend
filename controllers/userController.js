...
// @desc    Get newsletter preference
// @route   GET /api/users/newsletter-preference
// @access  Private
export const getNewsletterPreference = async (req, res) => {
  try {
    const user = await User.findById(req.user._id).select('newsletterSubscribed name email');
    if (!user) {
      return res.status(404).json({
        success: false,
        message: "User not found",
    ...
    console.error("Get newsletter preference error:", error);
    res.status(500).json({
      success: false,
      message: "Server error",
    });
...
// @desc    Update newsletter preference
// @route   PUT /api/users/newsletter-preference
// @access  Private
export const updateNewsletterPreference = async (req, res) => {
  try {
    ...
    if (typeof subscribed !== 'boolean') {
      return res.status(400).json({
        success: false,
        message: "Subscribed field must be a boolean",
      });
    }
